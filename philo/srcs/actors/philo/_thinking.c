...
/*   Updated: 2025/06/24 22:24:34 by ttsubo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	_thinking(t_philo_actor *self)
{
	...
	if (!self->can_eat)
		return ;
	if (!self->has_l_fork)
		l_tell(self->l_fork, msg_new(REQUEST_FORK, self->base));
	if (!self->has_r_fork)
		r_tell(self->r_fork, msg_new(REQUEST_FORK, self->base));
	if (self->has_l_fork && self->has_r_fork)
	{
		self->sts = PHILO_STS_EATING;
		self->hp.now = self->hp.max;
		self->sv->tell(self->sv, msg_new(MONITOR_EATING, self->base));
	}
}
